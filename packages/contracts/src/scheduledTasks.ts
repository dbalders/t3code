...
const decodeRRuleConfigString = Schema.decodeUnknownSync(
  Schema.fromJsonString(ScheduledTaskRRuleConfig),
);

function isValidIsoDateTimeString(value: string) {
  return Option.isSome(DateTime.make(value));
}

function isValidRRuleConfigString(value: string) {
  try {
    decodeRRuleConfigString(value);
    return true;
  } catch {
    return false;
  }
...
function validateSchedulePair(input: {
  readonly scheduleKind: ScheduledTaskScheduleKind;
  readonly scheduleValue: string;
}) {
  if (input.scheduleKind === "once") {
    return isValidIsoDateTimeString(input.scheduleValue)
      ? undefined
      : { path: ["scheduleValue"], issue: "once schedules require a valid ISO date-time" };
  }
  return isValidRRuleConfigString(input.scheduleValue)
    ? undefined
    : { path: ["scheduleValue"], issue: "rrule schedules require a valid recurrence config" };
}

function validateTargetForKind(input: {
  readonly kind: ScheduledTaskKind;
  readonly targetThreadId?: ThreadId | null | undefined;
}) {
  if (input.kind === "thread" && input.targetThreadId == null) {
    return { path: ["targetThreadId"], issue: "thread automations require a target thread" };
  }
  if (input.kind === "standalone" && input.targetThreadId != null) {
    return { path: ["targetThreadId"], issue: "standalone automations cannot target a thread" };
  }
  return undefined;
}
...
const ScheduledTaskMutationBaseStruct = Schema.Struct({
  name: TrimmedNonEmptyString,
  kind: ScheduledTaskKind,
  projectId: ProjectId,
...
export const ScheduledTaskMutationBase = ScheduledTaskMutationBaseStruct.check(
  Schema.makeFilter<typeof ScheduledTaskMutationBaseStruct.Type>(
    (input) => validateTargetForKind(input) ?? validateSchedulePair(input),
  ),
);
...
const ScheduledTaskUpdatePatchStruct = Schema.Struct({
...
const ScheduledTaskUpdatePatch = ScheduledTaskUpdatePatchStruct.check(
  Schema.makeFilter<typeof ScheduledTaskUpdatePatchStruct.Type>((patch) => {
    if (patch.kind === "thread" && patch.targetThreadId == null) {
      return { path: ["targetThreadId"], issue: "thread automations require a target thread" };
    }
    if (patch.kind === "standalone" && patch.targetThreadId != null) {
      return { path: ["targetThreadId"], issue: "standalone automations cannot target a thread" };
    }
    if (patch.scheduleKind !== undefined && patch.scheduleValue !== undefined) {
      return validateSchedulePair({
        scheduleKind: patch.scheduleKind,
        scheduleValue: patch.scheduleValue,
      });
    }
    return undefined;
  }),
...
export const ScheduledTaskUpdateInput = Schema.Struct({
  id: ScheduledTaskId,
  patch: ScheduledTaskUpdatePatch,
});
