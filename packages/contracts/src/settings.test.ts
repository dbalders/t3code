...
describe("ServerSettings.providerInstances (slice-2 invariant)", () => {
  ...
  it("decodes a fully empty config (legacy on-disk shape) without complaint", () => {
    ...
    expect(decoded.providers.opencode.customModels).toEqual(["ucsd/deepseek-v4-flash"]);
    expect(decoded.textGenerationModelSelection.instanceId).toBe("opencode");
    expect(decoded.textGenerationModelSelection.model).toBe("ucsd/deepseek-v4-flash");
  });
