...
This guide is for people who want to use more than one Claude setup in TritonAI Harness.
...
In TritonAI Harness Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means TritonAI Harness uses your normal home directory.
...
Then add another Claude provider in TritonAI Harness:
...
TritonAI Harness only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so TritonAI Harness keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in TritonAI Harness Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. TritonAI Harness stores the value as a server secret and does not
...
TritonAI Harness does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in TritonAI Harness.
