...
This guide is for people who want to use more than one Claude setup in TritonGPT Code.
...
In TritonGPT Code Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means TritonGPT Code uses your normal home directory.
...
Then add another Claude provider in TritonGPT Code:
...
TritonGPT Code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so TritonGPT Code keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in TritonGPT Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. TritonGPT Code stores the value as a server secret and does not
...
TritonGPT Code does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in TritonGPT Code.
