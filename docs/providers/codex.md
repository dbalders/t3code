...
This guide is for people who want to use more than one Codex account in TritonGPT Code.
...
In TritonGPT Code Settings, name it something obvious:
...
In TritonGPT Code Settings, add another Codex provider:
...
TritonGPT Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, TritonGPT Code treats it
