...
interface TokenCacheWithClear {
  readonly deleteToken?: (key: string) => Promise<void>;
  readonly saveToken?: (key: string, token: string) => Promise<void>;
}
...
async function syncNativeSession(sessionId: string): Promise<void> {
  ...
  const cache = tokenCache as TokenCacheWithClear | null | undefined;
  if (nativeClientToken) {
    const saveToken = cache?.saveToken;
    if (saveToken) {
      try {
  ...
  } else if (cache?.deleteToken) {
    try {
      await cache.deleteToken(CLERK_CLIENT_JWT_KEY);
    } catch (cause) {
      throw new NativeClerkAuthError({
        message: "Could not clear native Clerk client token.",
        cause,
      });
    }
  }
