...
const make = Effect.gen(function* () {
  ...
  const ensureRun = (task: ScheduledTask, scheduledFor: string, createdAt: string) =>
    Effect.gen(function* () {
      ...
      yield* runRepository.upsert(run).pipe(Effect.mapError(mapRepositoryError("run create")));
      return run;
    });
  ...
  const runDueTasks: ScheduledTaskServiceShape["runDueTasks"] = () =>
    Effect.gen(function* () {
      ...
      yield* Effect.forEach(tasks, (task) => runDueTask(task, currentTime), { concurrency: 1 });
    });
