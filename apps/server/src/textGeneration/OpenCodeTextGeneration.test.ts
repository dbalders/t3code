...
function startFakeOpenCodeServer(binaryPath: string) {
...
const OpenCodeRuntimeTestDouble: OpenCodeRuntimeShape = {
  startOpenCodeServerProcess: ({ binaryPath }) => startFakeOpenCodeServer(binaryPath),
  connectToOpenCodeServer: ({ binaryPath, serverUrl }) =>
    serverUrl
      ? Effect.succeed({
          url: serverUrl,
          exitCode: null,
      ...
      : startFakeOpenCodeServer(binaryPath).pipe(
          Effect.map((server) => ({
            url: server.url,
...
it.layer(OpenCodeTextGenerationTestLayer)("OpenCodeTextGeneration", (it) => {
  it.effect("releases the runtime lease as soon as local requests drain", () =>
