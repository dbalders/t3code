...
const makeScheduledTaskRunRepository = Effect.gen(function* () {
  ...
  const claimQueuedRunRow = SqlSchema.findOneOption({
    Request: ClaimScheduledTaskRunInput,
    Result: ScheduledTaskRun,
    execute: ({ id, threadId, messageId, startedAt, updatedAt }) =>
      sql`
  ...
  const claimQueued: ScheduledTaskRunRepositoryShape["claimQueued"] = (input) =>
    claimQueuedRunRow(input).pipe(
      Effect.mapError(toPersistenceSqlError("ScheduledTaskRunRepository.claimQueued:query")),
    );
