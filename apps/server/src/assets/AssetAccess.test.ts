...
describe("AssetAccess", () => {
  it.effect("issues workspace URLs that resolve the entry file and sibling assets", () =>
    Effect.gen(function* () {
      ...
      const realHtmlPath = yield* fileSystem.realPath(htmlPath);
      const realCssPath = yield* fileSystem.realPath(cssPath);
  ...
  it.effect("issues project favicon capabilities with a signed fallback", () =>
    Effect.gen(function* () {
      ...
      const realFaviconPath = yield* fileSystem.realPath(faviconPath);
      ...
      ).toEqual({ kind: "file", path: realFaviconPath });
