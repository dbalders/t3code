...
const CREATE_VIA_CHAT_PROMPT =
  "I want to set up an automation. Briefly explain how automations work in TritonAI Harness, then ask me a few questions to figure out what I'd like TritonAI Harness to do and when it should run.";

const DEFAULT_PROVIDER_DRIVER_KIND = ProviderDriverKind.make("codex");

function getFallbackModelForProvider(provider: ProviderDriverKind): string {
...
function createFallbackModelSelection(): ModelSelection {
  return {
    ...
    model: getFallbackModelForProvider(DEFAULT_PROVIDER_DRIVER_KIND),
  };
}
...
function fallbackModelSelectionFromEntries(
  ...
  return {
    ...
    model:
      ...
      entry.models[0]?.slug ??
      getFallbackModelForProvider(entry.driverKind),
  };
}
...
function resolveModelSelectionForEntries(input: {
  ...
  const model =
    ...
    getFallbackModelForProvider(selectedEntry.driverKind);
...
function formatSchedule(task: ScheduledTask): string {
  if (task.scheduleKind === "once") {
    return `Once at ${formatDateTime(task.scheduleValue, task.timezone)}`;
  }
  const config = parseRRuleConfig(task);
  if (!config) return "Custom schedule";
  const time = formatTime(config.dtStart, task.timezone);
...
function buildSchedule(form: AutomationFormState): {
  ...
  const startAtIso = datetimeLocalToIsoInTimeZone(form.startAt, form.timezone);
  if (!startAtIso) {
...
function formStateFromTask(task: ScheduledTask): AutomationFormState {
  ...
  const fallbackStartAt =
    startAtIso !== undefined
      ? (datetimeLocalFromIsoInTimeZone(startAtIso, task.timezone) ?? defaultStartAt())
      : defaultStartAt();
  const fallbackDay = Number.parseInt(fallbackStartAt.slice(8, 10), 10);
  const monthDay =
    config?.byMonthDay?.[0] ?? (Number.isFinite(fallbackDay) ? fallbackDay : new Date().getDate());
...
function AutomationDetail({
  ...
  const saveScheduleDraft = useCallback(
    (next: AutomationFormState) => {
      try {
        const timezone = next.timezone.trim() || task.timezone;
        const scheduleInput = { ...next, timezone };
        onUpdate(task, {
          ...buildSchedule(scheduleInput),
          timezone,
        ...
        <aside className="border-border/70 lg:border-l lg:pl-8">
          <div className="space-y-8">
            <section className="space-y-3">
              ...
              <DetailRow label="Next run" value={formatDateTime(task.nextRunAt, task.timezone)} />
              <DetailRow label="Last ran" value={formatDateTime(task.lastRunAt, task.timezone)} />
            </section>
...
function AutomationCreateDialog({
  ...
  const formTimezone = form.timezone.trim();
  const scheduleTimeLabel =
    formatTime(
      formTimezone ? datetimeLocalToIsoInTimeZone(form.startAt, formTimezone) : null,
...
export function AutomationsSettingsPanel() {
  ...
  const rawProviders = useAtomValue(primaryServerProvidersAtom);
  const providers = useMemo(
    () => rawProviders.filter((provider) => getDriverOption(provider.driver) !== undefined),
    [rawProviders],
  );
  ...
  const listScheduledTasks = useAtomCommand(serverEnvironment.scheduledTasksList, {
    label: "scheduled tasks list",
    reportFailure: false,
  });
  const createScheduledTask = useAtomCommand(serverEnvironment.scheduledTasksCreate, {
    label: "scheduled task create",
    reportFailure: false,
  });
  const updateScheduledTask = useAtomCommand(serverEnvironment.scheduledTasksUpdate, {
    label: "scheduled task update",
    reportFailure: false,
  });
  const deleteScheduledTask = useAtomCommand(serverEnvironment.scheduledTasksDelete, {
    label: "scheduled task delete",
    reportFailure: false,
  });
  const pauseScheduledTask = useAtomCommand(serverEnvironment.scheduledTasksPause, {
    label: "scheduled task pause",
    reportFailure: false,
  });
  const resumeScheduledTask = useAtomCommand(serverEnvironment.scheduledTasksResume, {
    label: "scheduled task resume",
    reportFailure: false,
  });
  const runScheduledTaskNow = useAtomCommand(serverEnvironment.scheduledTasksRunNow, {
    label: "scheduled task run now",
    reportFailure: false,
  });
  const listScheduledTaskRuns = useAtomCommand(serverEnvironment.scheduledTaskRunsList, {
    label: "scheduled task runs list",
    reportFailure: false,
  });
  const environmentProjects = useProjects();
  const environmentThreads = useThreadShells();
  const projects = useMemo(
    () =>
      environmentProjects
        .filter((project) => project.environmentId === primaryEnvironmentId)
        .map((project) => ({ ...project, name: project.title })),
    [environmentProjects, primaryEnvironmentId],
  );
  const allThreads = useMemo(
    () => environmentThreads.filter((thread) => thread.environmentId === primaryEnvironmentId),
    [environmentThreads, primaryEnvironmentId],
  );
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      const result = unwrapAtomCommandResult(
        await listScheduledTasks({ environmentId: primaryEnvironmentId, input: {} }),
      );
  ...
  }, [listScheduledTasks, primaryEnvironmentId]);
  ...
  const loadRuns = useCallback(
      ...
      try {
        ...
        const result = unwrapAtomCommandResult(
          await listScheduledTaskRuns({
            ...
            input: { taskId: taskId as ScheduledTask["id"] },
          }),
        );
    ...
    [listScheduledTaskRuns, primaryEnvironmentId],
  );
  ...
  const submitCreate = useCallback(async () => {
    ...
    setError(null);
    try {
      if (!primaryEnvironmentId) throw new Error("No active TritonAI environment.");
      ...
      const taskTimezone = form.timezone.trim() || timezone;
      const schedule = buildSchedule({ ...form, timezone: taskTimezone });
      const result = unwrapAtomCommandResult(
  ...
  const updateTaskPatch = useCallback(
    ...
    [enqueueTaskOperation, primaryEnvironmentId, updateScheduledTask],
  );
  ...
  const mutateTask = useCallback(
    async (task: ScheduledTask, action: "pause" | "resume" | "run" | "delete") => {
      ...
      try {
        await enqueueTaskOperation(task.id, async () => {
          if (!primaryEnvironmentId) throw new Error("No active TritonAI environment.");
          const target = { environmentId: primaryEnvironmentId, input: { id: task.id } };
          if (action === "pause") {
            unwrapAtomCommandResult(await pauseScheduledTask(target));
          }
          if (action === "resume") {
            unwrapAtomCommandResult(await resumeScheduledTask(target));
          }
          if (action === "run") {
            unwrapAtomCommandResult(await runScheduledTaskNow(target));
          }
          if (action === "delete") {
            unwrapAtomCommandResult(await deleteScheduledTask(target));
          }
          await loadTasks();
